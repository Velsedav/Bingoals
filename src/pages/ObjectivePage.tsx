...
import { clamp01, daysAgo } from "../lib/format";
...
export default function ObjectivePage() {
  ...
  const [timeMap, setTimeMap] = useState<Map<string, { total_ms: number; last_end: number | null }>>(
    new Map()
  );

  // ✅ Only one slideshow plays at a time (per objective page)
  const [playingSubId, setPlayingSubId] = useState<string | null>(null);

  const [addOpen, setAddOpen] = useState(false);

  // timer state: only 1 running at a time (within this page)
  ...
  async function reload() {
    ...
    // if the playing subobjective was deleted, stop slideshow
  ...
  // best-effort auto-stop if user navigates away
  useEffect(() => {
    return () => {
    ...
    <div className="page">
      ...
      <div className="panel">
        ...
        <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
          <div className="muted">
            Goal: {obj.goal_target ?? "—"} {obj.goal_unit ?? ""}
          </div>
...
function AddSubobjectiveModal(props: {
  ...
  return (
    <Modal open={props.open} title="Add subobjective" onClose={props.onClose}>
      <div className="form">
        ...
        <input
          ...
          placeholder="e.g., Book: The Stranger"
        />

        <label htmlFor="sub-total">Total (optional, supports chapters like 12)</label>
        <input id="sub-total" type="number" value={total} onChange={(e) => setTotal(Number(e.target.value))} />

        <label htmlFor="sub-unit">Unit</label>
        <input
          id="sub-unit"
          value={unit}
          onChange={(e) => setUnit(e.target.value)}
          placeholder="chapters / lessons / etc."
        />
    ...
    <div className={`panel ${autoDone ? "panelDone" : ""} ${isRunning ? "panelRecording" : ""}`}>
      <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
        <input
          ...
          value={s.title}
          onChange={(e) =>
            setSubs((prev) => prev.map((x) => (x.id === s.id ? { ...x, title: e.target.value } : x)))
          }
          onBlur={async () => {
        ...
        <div className="mini">
          ...
          <div className="row" style={{ gap: 8 }}>
            <input
              ...
              onChange={(e) => {
                const v = Number(e.target.value);
                setSubs((prev) => prev.map((x) => (x.id === s.id ? { ...x, progress_current: v } : x)));
              }}
              onBlur={async () => {
                const fresh = subs.find((x) => x.id === s.id);
                if (!fresh) return;
                ...
                await updateSubobjective(s.id, {
                  progress_current: fresh.progress_current,
                  is_done: hasTarget ? (autoDone ? 1 : 0) : fresh.is_done
                });
            ...
            <input
              ...
              onChange={(e) => {
                const v = Number(e.target.value);
                setSubs((prev) => prev.map((x) => (x.id === s.id ? { ...x, target_total: v } : x)));
              }}
              onBlur={async () => {
                const fresh = subs.find((x) => x.id === s.id);
                if (!fresh) return;
                ...
                const { hasTarget, autoDone } = computeAutoDone(fresh);
                await updateSubobjective(s.id, {
                  target_total: fresh.target_total,
                  is_done: hasTarget ? (autoDone ? 1 : 0) : fresh.is_done
                });
            ...
            <input
              ...
              placeholder="unit"
              onChange={(e) =>
                setSubs((prev) => prev.map((x) => (x.id === s.id ? { ...x, unit: e.target.value } : x)))
              }
              onBlur={async () => {
                const fresh = subs.find((x) => x.id === s.id);
        ...
        <div className="row" style={{ gap: 8, marginLeft: "auto" }}>
          {(s.target_total ?? 0) <= 0 && (
            <button
              ...
              title="Toggle done (only used when target is 0)"
            >
      ...
      <div className="memories">
        <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div className="row" style={{ gap: 8 }}>
            ...
            {/* ✅ Play/Pause slideshow (only if 2+ items) */}
            <button
              className="btn"
              disabled={subMedia.length < 2}
              title="Play/Pause diaporama (only one plays at a time)"
      ...
      <TimeEditModal
        ...
        onSave={async (ms) => {
          ...
          await setSubobjectiveTotalTime(s.id, ms);
          await reload();
        }}
...
function Slideshow(props: {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ...
      { threshold: 0.35, root: null, rootMargin: "200px 0px" }
    );
  ...
  // Autoplay only if playing + in view + 2+ items
  useEffect(() => {
  ...
  return (
    <div className="slideshow" ref={rootRef}>
      <div className="slide">
        ...
        {/* key=item.id => re-triggers fade animation */}
      ...
      <div className="row" style={{ justifyContent: "space-between", marginTop: 8 }}>
        <button
          ...
          onClick={() => {
            props.onRequestStop();
            setI((x) => (x - 1 + props.items.length) % props.items.length);
          }}
        >
          Prev
        </button>
...
function TimerDisplay(props: { totalMs: number; isRunning: boolean; startedAt: number | null }) {
  ...
  useEffect(() => {
    if (!props.isRunning || !props.startedAt) {
      setDisplayMs(props.totalMs);
      return;
    }

    // Start interval
    const id = window.setInterval(() => {
      const extra = Math.max(0, Date.now() - props.startedAt!);
      setDisplayMs(props.totalMs + extra);
    }, 500);

    // Initial render catchup
    const initialExtra = Math.max(0, Date.now() - props.startedAt!);
    setDisplayMs(props.totalMs + initialExtra);
...
function TimeEditModal(props: {
  ...
  return (
    <Modal open={props.open} title="Edit time" onClose={props.onClose}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <input
              ...
              onChange={(e) => {
                ...
                setH(val);
                if (val.length === 3) mRef.current?.select();
              }}
              onKeyDown={(e) => {
                if (e.key === "Enter") save();
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <input
              ...
              onChange={(e) => {
                ...
                setM(val);
                if (val.length === 2) sRef.current?.select();
              }}
              onKeyDown={(e) => {
                if (e.key === "Backspace" && m === "") hRef.current?.select();
                if (e.key === "Enter") save();
                if (e.key === "Escape") props.onClose();
              }}
            />
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Backspace" && s === "") mRef.current?.select();
                if (e.key === "Enter") save();
                if (e.key === "Escape") props.onClose();
              }}
            />
