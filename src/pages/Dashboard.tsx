...
import { computeObjectivePercent } from "../lib/progress";
...
// cache module (persiste tant que l’app tourne)
...
function lastStatus(days: number | null, freqDays: number | null) {
  ...
  const greenCut = Math.floor(freqDays * 0.25); // 25% window = green
...
export default function Dashboard() {
    ...
    try {
      ...
      const out: Cell[] = rows.map((r) => {
        ...
        if (objective) {
          ...
          const lastSubUpdate =
            subs.length === 0 ? 0 : subs.reduce((m, s) => Math.max(m, s.updated_at ?? 0), 0);
  ...
  const viewCells = useMemo(() => {
    const clone = [...cells];
    clone.sort((a, b) => {
      // 1) keep empty slots at the bottom
      ...
      // 2) pinned-to-bottom always last (never inverted)
      const ap = !!a.objective!.pin_bottom;
      const bp = !!b.objective!.pin_bottom;
      if (ap !== bp) return ap ? 1 : -1;

      // 3) Focus: oldest activity FIRST
      // If last_progress_at is null -> treat as VERY old (Infinity) so it goes to the top
      ...
      // smaller timestamp = older -> should come first
...
async function fetchSubobjectivesByObjective(objectiveIds: string[]) {
  ...
  const q = `
    SELECT *
    FROM subobjectives
    WHERE objective_id IN (${objectiveIds.map(() => "?").join(",")})
...
function CreateObjectiveModal(props: { slotIndex: number | null; onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <Modal open={open} title="Create objective" onClose={props.onClose}>
      <div className="form">
        <label htmlFor="create-title">Title</label>
        <input id="create-title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Learn 10 recipes" />

        <label htmlFor="create-kind">Goal type</label>
        <select id="create-kind" value={kind} onChange={(e) => setKind(e.target.value as any)}>
        ...
        <label htmlFor="create-target">Goal target</label>
        <input id="create-target" type="number" value={target} onChange={(e) => setTarget(Number(e.target.value))} />

        <label htmlFor="create-unit">Unit</label>
        <input id="create-unit" value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="books / € / % / wpm" />
...
function EditObjectiveModal(props: { objective: Objective | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <Modal open={open} title="Edit objective" onClose={props.onClose}>
      <div className="form">
        <label htmlFor="edit-title">Title</label>
        <input id="edit-title" value={title} onChange={(e) => setTitle(e.target.value)} />

        <label htmlFor="edit-kind">Goal type</label>
        <select id="edit-kind" value={kind} onChange={(e) => setKind(e.target.value as any)}>
          <option value="count">Count — track items completed (e.g., 12 books)</option>
          <option value="metric">Metric — track a measured value (e.g., 1000 km)</option>
          <option value="amount">Amount — track a sum (e.g., €500 saved)</option>
          <option value="manual">Manual — set progress percentage directly</option>
        ...
        <label htmlFor="edit-target">Goal target</label>
        <input id="edit-target" type="number" value={target} onChange={(e) => setTarget(Number(e.target.value))} />
        ...
        {(kind === "metric" || kind === "amount" || kind === "manual") && (
          <>
            ...
            <input id="edit-current" type="number" value={currentValue} onChange={(e) => setCurrentValue(Number(e.target.value))} />
          </>
        )}

        <label htmlFor="edit-freq">Frequency (days) — optional (ex: 1, 7, 14, 31, 182)</label>
        <input
          id="edit-freq"
          type="number"
          min={0}
        ...
        <div className="row" style={{ flexWrap: "wrap" }}>
          ...
          <button className="btn" onClick={() => setCoverData(null)}>
            Remove cover
          </button>
        </div>
...
const DashboardCard = memo(function DashboardCard({
  ...
  return (
    <div className="cardWrap">
      <div
        ...
        onClick={() => nav(`/objective/${c.objective!.id}`)}
        onKeyDown={(e) => { if (e.key === "Enter") nav(`/objective/${c.objective!.id}`); }}
      >
        <div className="cardTitle">{c.objective!.title}</div>
        <div className="cardMeta">
          <div>
            <span className="muted">Time:</span> {formatDuration(c.total_ms)}
          </div>
          <div>
          ...
          <div>
            <span className="muted">Progress:</span> {percentText}
          </div>
        </div>
      ...
      <div className="cardActions" onClick={(e) => e.stopPropagation()}>
        <button
          ...
          onClick={async () => {
            await updateObjective(c.objective!.id, { pin_bottom: pinned ? 0 : 1 });
            await load();
          }}
        >
          {pinned ? "⇧" : "⇩"}
        ...
        <button className="iconBtn" title="Edit objective" onClick={() => setEditObj(c.objective!)}>
          ✎
        </button>
      </div>
    </div>
  ...
  // If the cell object hasn't structurally changed in our mapped layout array, skip re-render.
  // We can just rely on the reference equality created in `load()` or do a deep equality check:
