...
import { DEFAULT_QUOTES } from "./lib/defaultQuotes";
...
export type Objective = {
  ...
  cover_data: string | null; // dataURL
  current_value: number;
  created_at: number;
  updated_at: number;
  pin_bottom: number; // 0/1
...
export type MediaItem = {
  ...
  kind: "quote" | "image";
  data: string; // quote text OR image dataUrl
  created_at: number;
};
...
async function ensureSchema(db: Database) {
  ...
  // ---- Perf ----
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_subobjectives_objective_id ON subobjectives(objective_id);`);
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_time_sessions_subobjective_id ON time_sessions(subobjective_id);`);
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_media_items_subobjective_id ON media_items(subobjective_id);`);
  ...
  // ---- Seeds (IMPORTANT: before any early return) ----

  // seed quotes if empty
  const qCount = await db.select<{ c: number }[]>("SELECT COUNT(*) as c FROM quotes;");
  if ((qCount?.[0]?.c ?? 0) === 0) {
    for (const text of DEFAULT_QUOTES) {
      await db.execute(`INSERT INTO quotes (id, text, created_at) VALUES (?, ?, ?)`, [
        crypto.randomUUID(),
        text,
        now()
      ]);
  ...
  // seed objectives only if none exist
  const objCount = await db.select<{ c: number }[]>("SELECT COUNT(*) as c FROM objectives;");
  if ((objCount?.[0]?.c ?? 0) !== 0) return;

  const seed = [
    { title: "Listen to 24 Albums", goal_kind: "count", goal_target: 24, goal_unit: "albums" },
    { title: "Read 6 Books", goal_kind: "count", goal_target: 6, goal_unit: "books" },
    { title: "Watch 12 French Films", goal_kind: "count", goal_target: 12, goal_unit: "films" },
    { title: "Learn 6 Piano Pieces", goal_kind: "count", goal_target: 6, goal_unit: "pieces" },
    { title: "Finish 12 Games", goal_kind: "count", goal_target: 12, goal_unit: "games" },
    { title: "Reach 60 WPM (Touch Typing)", goal_kind: "metric", goal_target: 60, goal_unit: "wpm" },
    { title: "Learn Excel", goal_kind: "manual", goal_target: 100, goal_unit: "%" },
    { title: "Study Maths (BTS SIO SISR)", goal_kind: "manual", goal_target: 100, goal_unit: "%" },
    { title: "Invest 1000€", goal_kind: "amount", goal_target: 1000, goal_unit: "€" },
    { title: "Organize Digital Files (Zero Inbox)", goal_kind: "manual", goal_target: 100, goal_unit: "%" },
    { title: "Cook 12 New Recipes (8 Vegan)", goal_kind: "count", goal_target: 12, goal_unit: "recipes" },
    { title: "Learn 3 Couple Dances", goal_kind: "count", goal_target: 3, goal_unit: "dances" }
  ] as const;
  ...
  for (let i = 0; i < seed.length; i++) {
    ...
    await db.execute(
      `INSERT INTO objectives
       ...
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 0, NULL)`,
      [id, s.title, s.goal_kind, s.goal_target, s.goal_unit, null, t, t]
    );

    await db.execute(`UPDATE slots SET objective_id = ? WHERE slot_index = ?`, [id, i]);
  }
}
...
export async function listSlotsWithObjectives() {
  ...
  const rows = await db.select<(SlotRow & Partial<Objective>)[]>(
...
export async function updateSubobjective(id: string, patch: Partial<Subobjective>) {
  ...
  await db.execute(
    ...
    [
      merged.title,
      merged.note ?? null,
      merged.target_total ?? null,
      merged.progress_current ?? 0,
      merged.unit ?? null,
      merged.is_done ?? 0,
      merged.updated_at,
      id
...
export async function addQuote(subobjectiveId: string, quoteText: string) {
  ...
  const id = crypto.randomUUID();
  await db.execute(
    `INSERT INTO media_items (id, subobjective_id, kind, data, created_at)
     VALUES (?, ?, 'quote', ?, ?)`,
...
export async function listDashboardRows(): Promise<DashboardRow[]> {
  ...
  // ✅ include pin_bottom + frequency_days
...
// ---------- Quotes (editable in app) ----------

export async function addQuoteText(text: string) {
  const db = await getDb();
  await db.execute(`INSERT INTO quotes (id, text, created_at) VALUES (?, ?, ?)`, [crypto.randomUUID(), text, now()]);
}
...
export async function updateQuoteText(id: string, text: string) {
  const db = await getDb();
  await db.execute(`UPDATE quotes SET text = ? WHERE id = ?`, [text, id]);
}

export async function resetQuotesToDefaults() {
  const db = await getDb();
...
async function getOrCreateObjectiveTimeBucket(objectiveId: string) {
  const db = await getDb();

  const existing = await db.select<Subobjective[]>(
    `SELECT * FROM subobjectives WHERE objective_id = ? AND title = ? LIMIT 1`,
    [objectiveId, "__Time adjustments"]
  );
  ...
  await db.execute(
    ...
    [id, objectiveId, "__Time adjustments", "Internal bucket used for manual time edits.", t, t]
  );

  return id;
}

export async function setObjectiveTotalTime(objectiveId: string, desiredTotalMs: number) {
  const stats = await getTimeStatsForObjective(objectiveId);
  const current = stats.total_ms ?? 0;
  const delta = Math.trunc(desiredTotalMs - current);
  if (delta === 0) return;

  const bucketSubId = await getOrCreateObjectiveTimeBucket(objectiveId);
  await addManualTimeDelta(bucketSubId, delta);
}
...
export async function listQuotes() {
  const db = await getDb();
  return db.select<QuoteRow[]>(`SELECT * FROM quotes ORDER BY created_at ASC`);
}

export async function addFooterQuote(text: string) {
  const db = await getDb();
  const t = now();
  const id = crypto.randomUUID();
  await db.execute(`INSERT INTO quotes (id, text, created_at) VALUES (?, ?, ?)`, [id, text, t]);
  return id;
}

export async function deleteFooterQuote(id: string) {
  const db = await getDb();
  await db.execute(`DELETE FROM quotes WHERE id = ?`, [id]);
}
...
export async function exportBackupTo(path: string) {
  const db = await getDb();
  try {
...
export async function importBackupFrom(path: string) {
  await closeDb();
